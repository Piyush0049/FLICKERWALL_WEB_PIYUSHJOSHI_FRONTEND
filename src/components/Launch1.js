...
function App() {
    ...
    const SITE = "https://flickerwall-web-piyushjoshi-backend.onrender.com"
    ...
    const orgcodeverification = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${SITE}/screens/findscreenorgcode`, { orgcode: orgcode, screenname: screenname }, { headers: header });
            if (response.data.success) {
                ...
                toast.error(response.data.message); // Use toast.error for error message
                navigate("/fail");
            }
        } catch (error) {
            console.error("Error verifying organization code:", error);
            toast.error("Error verifying organization code");
            navigate("/fail");
        }
    };
